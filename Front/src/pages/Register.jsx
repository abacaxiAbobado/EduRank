...
export default function Register() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const data = await registerService(form.name, form.username, form.password);
      login(data);
      navigate('/dashboard');
    } catch (err) {
  ...
  const fields = [
    ...
    ['username', 'Nome de usuário', 'text', 'ex: joao_silva'],
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 20, padding: 36 }}>
          <h2 style={{ fontFamily: 'var(--font-heading)', fontWeight: 800, marginBottom: 24 }}>Criar conta</h2>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {fields.map(([key, label, type, ph]) => (
              <div key={key} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                <input type={type} value={form[key]} onChange={e => setForm({ ...form, [key]: e.target.value })} placeholder={ph} required minLength={key === 'password' ? 6 : undefined}
